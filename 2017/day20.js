...
const particles = test ? testParticles : realParticles;

function getManhatten(particle, aspect) {
	const x = particle[aspect][0];
	const y = particle[aspect][1];
	const z = particle[aspect][2];
	return Math.abs(x) + Math.abs(y) + Math.abs(z);
}

const minForA = _.min(particles, particle => getManhatten(particle, 'a'));
// console.log(minForA);

function stepParticles(particles) {
	return _.map(particles, particle => {
		particle['v'][0] = particle['v'][0] + particle['a'][0];
		particle['v'][1] = particle['v'][1] + particle['a'][1];
		particle['v'][2] = particle['v'][2] + particle['a'][2];
		particle['p'][0] = particle['p'][0] + particle['v'][0];
...
while(count) {
	let hash = {};
	_.each(remainingParticles, (particle, index) => {
		const hashCode = particle['p'].join(',');
		if (hash[hashCode]) {
			hash[hashCode].push(index);
		} else {
			hash[hashCode] = [index];
		}
	});
	const collisions = _.chain(hash).filter(hashEntry => hashEntry.length > 1).flatten().value();
	remainingParticles = _.reject(remainingParticles, (p, i) => _.contains(collisions, i));
	remainingParticles = stepParticles(remainingParticles);
	count--;
}
console.log(remainingParticles.length);